...
import {startSync, syncStateStore, reservedDateTime} from "./";
import {State, SyncState, SyncResult} from "./sync";
...
let animationTimer: NodeJS.Timer | undefined = undefined;
...
const refreshMenu = () => {
    ...
    if (syncStateStore.getState().syncing.state === SyncState.Initial) {
        menuItemTemplate = {label: "同期を手動で開始する", type: "normal", click: () => {
            startSync();
        }};
    } else {
    ...
    const contextMenu: Electron.Menu = Menu.buildFromTemplate([
        menuItemTemplate,
        {type: "separator"},
        {label: "開く...", type: "submenu", submenu: [
            {label: "データフォルダを開く", type: "normal", click: () => shell.showItemInFolder(app.getPath("userData")) }
        ]},
