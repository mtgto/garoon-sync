...
export type Progress = {num: number, den: number}; // (numerator / denominator)
...
const reducer: Reducer<State> = (state: State = initialState, action: SyncActions): State => {
    switch (action.type) {
        case ActionTypes.StartFetchGaroon:
            return {...state, syncing: {...state.syncing, state: SyncState.FetchingGaroon, result: SyncResult.Unknown}};
        ...
        case ActionTypes.StartSyncGoogleCalendar:
            return {...state, syncing: {...state.syncing, state: SyncState.SyncingGoogleCalendar, result: SyncResult.Unknown}};
        ...
        case ActionTypes.EndSync:
            if (action.payload === SyncResult.Success) {
                ...
                return {...state, syncing: {...state.syncing, state: SyncState.Initial, result: action.payload}, progress: {num: 0, den: 1}, lastSyncTime: moment()};
            } else {
                return {...state, syncing: {...state.syncing, state: SyncState.Initial, result: action.payload}, progress: {num: 0, den: 1}};
            }
        default:
            return state;
    }
