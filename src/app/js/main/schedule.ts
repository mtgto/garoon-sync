 ...
 */
const googleDateTimeFromDateTime = (
    dateTime: DateTime,
...
export const fromGaroonSchedule = (json: any): Schedule => {
    ...
    const visibility: Visibility = event.attributes.public_type
        ? visibilityFromPublicType(event.attributes.public_type)
        : Visibility.Public;
    let start: DateTime;
    ...
    if (event.repeat_info) {
        ...
        if (event.when.date) {
            ...
            throw new Error(
                `Event ${event.attributes.id} isn't supported: Neither event.when.date nor event.when.datetime exists.`,
            );
        }
    } else {
        throw new Error(
